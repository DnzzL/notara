...
test.describe("Database Views", () => {
  ...
  const createDatabaseWithDateField = async (page: any) => {
    ...
    if (await fieldNameInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const typeSelect = page.locator("select").filter({ hasText: /text|number|select/i }).first();
  ...
  test("CR-1: Board → Calendar switching persists (does not revert to Board)", async ({ page }) => {
    ...
    const calendarTab = page.locator('[role="tab"]').filter({ hasText: "Calendar" });
  ...
  test("CR-2: Month navigation prev/next", async ({ page }) => {
    ...
    const noDateFieldMsg = page.getByText("Add a Date field to use the calendar view");
  ...
  test("CR-3: '+' on a day creates a record dialog", async ({ page }) => {
    ...
    const recordEntry = page.locator("button").filter({ hasText: "Calendar Created Event" });
