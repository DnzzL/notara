...
export function createPresenceService(opts: PresenceServiceOptions = {}) {
  ...
  function emit(ws: string, page: string, event: PresenceEvent, excludeUserId: string | null) {
    const subs = subscribers.get(key(ws, page));
    if (!subs) return;
    for (const s of subs) {
      if (excludeUserId !== null && s.userId === excludeUserId) continue;
      try { s.push(event); } catch { /* subscriber errors don't break the broadcaster */ }
    }
  }
  ...
  return {
    heartbeat(input: {
      ...
      if (!pageMap) { pageMap = new Map(); entries.set(k, pageMap); }
      const t = now();
      const prev = pageMap.get(input.user.id);
      const focusChanged = !prev || prev.focusedBlockId !== input.focusedBlockId;
      ...
      if (!prev || focusChanged) {
        emit(input.workspaceId, input.pageId, {
    ...
    lockHolder(workspaceId: string, pageId: string, blockId: string): string | null {
