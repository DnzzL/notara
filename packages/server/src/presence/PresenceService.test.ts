...
import { createPresenceService, type PresenceEvent } from "./PresenceService.js";
...
describe("PresenceService", () => {
  ...
  it("records a heartbeat and reports the user as present on the page", () => {
    ...
    svc.heartbeat({ workspaceId: WS, pageId: PAGE, user: ALICE, focusedBlockId: null });

    const present = svc.presence(WS, PAGE);
    expect(present).toEqual([{ userId: ALICE.id, name: ALICE.name, focusedBlockId: null }]);
  });

  it("dedupes presence by userId across multiple heartbeats", () => {
    const svc = createPresenceService({ now: clock.now });
    svc.heartbeat({ workspaceId: WS, pageId: PAGE, user: ALICE, focusedBlockId: null });
    svc.heartbeat({ workspaceId: WS, pageId: PAGE, user: ALICE, focusedBlockId: BLOCK_A });
  ...
  it("releases the lock when the user heartbeats with a different focus", () => {
    const svc = createPresenceService({ now: clock.now });
    svc.heartbeat({ workspaceId: WS, pageId: PAGE, user: ALICE, focusedBlockId: BLOCK_A });
    svc.heartbeat({ workspaceId: WS, pageId: PAGE, user: ALICE, focusedBlockId: BLOCK_B });
  ...
  it("notifies subscribers when a heartbeat changes presence on the page", () => {
    ...
    svc.heartbeat({ workspaceId: WS, pageId: PAGE, user: BOB, focusedBlockId: null });
  ...
  it("broadcasts block updates to subscribers of the page", () => {
    ...
    svc.broadcast(WS, PAGE, { type: "block.updated", actorUserId: ALICE.id, blockId: BLOCK_A, content: "<p>hi</p>" });
    expect(events).toEqual([
      { type: "block.updated", actorUserId: ALICE.id, blockId: BLOCK_A, content: "<p>hi</p>" },
    ]);
  });
