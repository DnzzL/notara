...
import { HashMap, Layer, List, LogLevel, Logger } from "effect";
...
export function reportError(err: unknown, context?: Record<string, unknown>): void {
  if (posthog) {
    try {
      ...
      console.error("[observability] posthog.captureException failed", captureErr);
    }
  }
 ...
 */
const jsonLogger = Logger.make(({ logLevel, message, date, spans, annotations }) => {
  const spanList = List.toArray(spans);
  const annoEntries = HashMap.toEntries(annotations);
  const payload: Record<string, unknown> = {
    ts: date.toISOString(),
    level: logLevel.label,
    msg: Array.isArray(message) ? message.map(String).join(" ") : String(message),
  };
  if (spanList.length > 0) payload.spans = spanList.map((s) => s.label);
  if (annoEntries.length > 0) {
    payload.annotations = Object.fromEntries(annoEntries.map(([k, v]) => [k, String(v)]));
  }
...
const levelFromEnv = (fallback: "Info" | "Debug") => {
  ...
  switch (raw) {
    case "All": case "Trace": case "Debug": case "Info":
    case "Warning": case "Error": case "Fatal": case "None":
