...
export async function restoreBackup(key: string): Promise<RestoreResult> {
  ...
  if (key.includes("..") || key.includes("\0")) throw new Error("Invalid backup key");
  if (!key.startsWith(prefix)) throw new Error("Backup key is outside the configured prefix");
  if (!/backup-.*\.zip$/.test(key)) throw new Error("Key does not look like a backup zip");
  ...
  if (!entries.some((e) => e.entryName === "platform.db")) {
    throw new Error("Backup zip does not contain platform.db — refusing to restore");
  }
  ...
  if (fs.existsSync(stagedPlatform)) fs.renameSync(stagedPlatform, platformDbPath);
  ...
  if (fs.existsSync(stagedWorkspaces)) fs.renameSync(stagedWorkspaces, workspacesDir);
  ...
  if (fs.existsSync(stagedAttachments)) fs.renameSync(stagedAttachments, attachmentsDir);
