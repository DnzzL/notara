...
type ViewDef = { name: string; type: "table" | "board"; groupByFieldName?: string };
type RecordDef = {
  title: string;
...
const h1 = (t: string): PlainBlock => ({ kind: "block", type: "heading1", content: `<h1>${t}</h1>` });
const h2 = (t: string): PlainBlock => ({ kind: "block", type: "heading2", content: `<h2>${t}</h2>` });
const p = (t: string): PlainBlock => ({ kind: "block", type: "paragraph", content: `<p>${t}</p>` });
...
export const TEMPLATES: TemplateDef[] = [
  // ── 1. Engineering Wiki ───────────────────────────────────────────────────
  {
    ...
    blocks: [
      h1("Engineering Wiki 📚"),
      p("Your team's home base for docs, decisions, and how-tos. Expand a section to read it, collapse it to scan. Deeper guides live as their own sub-pages."),
      {
        kind: "block",
        type: "toggle",
        content: TOGGLE(
      ...
      h2("📂 Sub-pages"),
      p("Deeper docs live as their own pages. Click in below, or browse the nested tree in the sidebar — sub-pages can nest as deep as you like."),
      { kind: "childLinks" },
    ],
    children: [
      {
        ...
        blocks: [
          h1("Runbooks 📕"),
          p("Step-by-step operational procedures. Open one below, or add your own as a new sub-page of this one."),
          h2("Procedures"),
          { kind: "childLinks" },
        ],
        children: [
          {
            ...
            blocks: [
              h1("Deploy to Production 🚀"),
              {
                kind: "block",
                type: "callout",
                content: CALLOUT("⚠️ Before you start", "CI is green on <code>main</code> and the deploy window is announced in the team channel."),
              },
              h2("Steps"),
              {
                kind: "block",
              ...
              },
              {
                kind: "block",
                type: "callout",
                content: CALLOUT("↩️ Rollback", "Re-deploy the previous tag: <code>npm run deploy:prod -- --tag vX.Y.(Z-1)</code>"),
              },
            ],
          },
          {
            ...
            blocks: [
              h1("Incident Response 🚨"),
              {
                kind: "block",
                type: "callout",
                content: CALLOUT("🔴 Severity", "SEV1 = full outage · SEV2 = major degradation · SEV3 = minor / workaround exists."),
              },
              h2("Playbook"),
              {
                kind: "block",
      ...
      },
      {
        ...
        blocks: [
          ...
          { kind: "block", type: "todo", content: TODO("Get access to the repo, CI, and the team channel") },
          { kind: "block", type: "todo", content: TODO("Set up your local environment (see Getting Started)") },
          { kind: "block", type: "todo", content: TODO("Read the Architecture section and the Runbooks") },
          { kind: "block", type: "todo", content: TODO("Ship a small PR to learn the workflow") },
          { kind: "block", type: "todo", content: TODO("Pair with a teammate on a real task") },
          {
            kind: "block",
            type: "toggle",
            content: TOGGLE("🔗 Handy links", UL(["Repo & PR guidelines", "Design system / component library", "On-call schedule"])),
          },
        ],
      },
    ],
  ...
  // ── 2. Project / Sprint Tracker ───────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      p("Track tasks, assign owners, and manage your sprint. Switch to the <strong>Board</strong> view for a Kanban layout."),
      h2("📂 Project docs"),
      p("Supporting docs live as sub-pages so the tracker stays focused on tasks:"),
      { kind: "childLinks" },
      { kind: "block", type: "divider", content: "" },
      h2("Sprints"),
      p("Group work into sprints. Tasks link here via a <strong>relation</strong>."),
      {
        kind: "database",
        name: "Sprints",
        titleLabel: "Sprint",
        fields: [
          { name: "Status", type: "select", options: ["Planned", "Active", "Done"] },
          { name: "Dates", type: "text" },
        ],
        views: [{ name: "Table", type: "table" }],
        records: [
          { title: "Sprint 1 — Foundation", values: { Status: "Done", Dates: "Weeks 1–2" } },
          { title: "Sprint 2 — Core features", values: { Status: "Active", Dates: "Weeks 3–4" } },
          { title: "Sprint 3 — Launch", values: { Status: "Planned", Dates: "Weeks 5–6" } },
        ],
      },
      h2("Tasks"),
      p("Each task links to its <strong>Sprint</strong> (a relation to the database above) and can point to a <strong>Spec</strong> page."),
      {
        kind: "database",
        name: "Tasks",
        titleLabel: "Task",
        fields: [
          { name: "Status", type: "select", options: ["Todo", "In Progress", "Done", "Blocked"] },
          { name: "Priority", type: "select", options: ["Low", "Medium", "High"] },
          { name: "Assignee", type: "select", options: ["Alex", "Sam", "Jordan"] },
        ...
        records: [
          { title: "Define project scope", values: { Status: "Done", Priority: "High", Assignee: "Alex" }, relations: { Sprint: ["Sprint 1 — Foundation"] }, pageLinks: { Spec: "Project Brief" } },
          { title: "Set up repository & CI", values: { Status: "Done", Priority: "High", Assignee: "Sam" }, relations: { Sprint: ["Sprint 1 — Foundation"] } },
          { title: "Design system architecture", values: { Status: "In Progress", Priority: "High", Assignee: "Alex" }, relations: { Sprint: ["Sprint 2 — Core features"] }, pageLinks: { Spec: "Project Brief" } },
          { title: "Implement core features", values: { Status: "In Progress", Priority: "Medium", Assignee: "Jordan" }, relations: { Sprint: ["Sprint 2 — Core features"] } },
          { title: "Write documentation", values: { Status: "Todo", Priority: "Medium", Assignee: "Sam" }, relations: { Sprint: ["Sprint 3 — Launch"] } },
          { title: "Deploy to staging", values: { Status: "Todo", Priority: "Medium" }, relations: { Sprint: ["Sprint 3 — Launch"] } },
          { title: "Ship v1.0", values: { Status: "Todo", Priority: "High" }, relations: { Sprint: ["Sprint 3 — Launch"] } },
        ],
      },
    ],
    children: [
      {
        ...
        blocks: [
          h1("Project Brief 📋"),
          h2("Goal"),
          p("What are we building and why? Capture the problem and the outcome that defines success."),
          h2("Scope"),
          { kind: "block", type: "bulletList", content: UL(["In scope: …", "Out of scope: …"]) },
          h2("Milestones"),
          { kind: "block", type: "numberedList", content: OL(["Kickoff & scope locked", "MVP feature-complete", "Beta with first users", "v1.0 launch"]) },
          h2("Stakeholders"),
          { kind: "block", type: "bulletList", content: UL(["Product: …", "Engineering: …", "Design: …"]) },
        ],
      },
    ],
  },
  ...
  // ── 3. Meeting Notes ─────────────────────────────────────────────────────
  {
    ...
    blocks: [
      h1("Meeting Notes 📋"),
      {
        kind: "block",
        type: "callout",
        content: CALLOUT("📅 Meeting info", "Date: &nbsp; · &nbsp; Participants: &nbsp; · &nbsp; Duration:"),
      },
      h2("Agenda"),
      ...
      h2("Decisions"),
      {
        kind: "block",
        type: "callout",
        content: CALLOUT("No decisions recorded yet.", "Add key decisions here as the meeting progresses."),
      },
      h2("Action Items"),
  ...
  // ── 4. Bug / Issue Tracker ────────────────────────────────────────────────
  {
    ...
    blocks: [
      h1("Bug Tracker 🐛"),
      p("Report issues, set severity and status, and track them to resolution. Your data stays on your server."),
      { kind: "block", type: "divider", content: "" },
      {
        kind: "database",
        name: "Issues",
        ...
        fields: [
          ...
          { name: "Reported by", type: "select", options: ["Alex", "Sam", "Jordan"] },
        ],
        views: [
        ...
        records: [
          { title: "Login fails on Safari with OAuth", values: { Severity: "High", Status: "Open", Component: "Auth", "Reported by": "Alex" } },
          { title: "Export button missing on mobile", values: { Severity: "Medium", Status: "In Progress", Component: "UI", "Reported by": "Sam" } },
          { title: "Search returns stale results after edit", values: { Severity: "High", Status: "Open", Component: "Search", "Reported by": "Jordan" } },
          { title: "Typo in onboarding error message", values: { Severity: "Low", Status: "Fixed", Component: "Onboarding", "Reported by": "Sam" } },
        ],
      },
    ],
  },
...
export const getTemplates = () => TEMPLATES.map(({ id, title, icon, description }) => ({ id, title, icon, description }));
  ...
  Effect.gen(function* () {
    ...
    if (def.children) {
      for (let ci = 0; ci < def.children.length; ci++) {
        const cid = yield* buildPage(def.children[ci], pageId, ci + 1, now, ctx);
        childIds.push(cid);
      }
    }
    ...
    for (const blockDef of def.blocks) {
      ...
      for (let fi = 0; fi < blockDef.fields.length; fi++) {
        ...
        const relationTargetDbId = field.relationTargetName
          ? (field.relationTargetName === blockDef.name ? dbId : ctx.dbByName.get(field.relationTargetName)?.id ?? null)
          : null;
        yield* sql`
      ...
      for (const view of blockDef.views) {
        const groupByFieldId = view.groupByFieldName ? (fieldIdMap[view.groupByFieldName] ?? null) : null;
        yield* sql`
      ...
      for (let ri = 0; ri < blockDef.records.length; ri++) {
        ...
        if (rec.values) {
          for (const [fieldName, rawValue] of Object.entries(rec.values)) {
            ...
            const value = field?.type === "multiSelect" ? JSON.stringify([rawValue]) : rawValue;
            yield* sql`
      ...
      for (let ri = 0; ri < blockDef.records.length; ri++) {
        ...
        for (const [fieldName, targetTitles] of Object.entries(rec.relations ?? {})) {
          const fieldId = fieldIdMap[fieldName];
          const targetName = blockDef.fields.find((f) => f.name === fieldName)?.relationTargetName;
          const targetMap = targetName ? ctx.dbByName.get(targetName)?.recordIdByTitle : undefined;
          if (!fieldId || !targetMap) continue;
          const ids = targetTitles.map((t) => targetMap.get(t)).filter((x): x is string => !!x);
        ...
        for (const [fieldName, pageTitle] of Object.entries(rec.pageLinks ?? {})) {
          const fieldId = fieldIdMap[fieldName];
...
export const createPageFromTemplate = (req: {
  ...
  Effect.gen(function* () {
    ...
    if (!template) return yield* Effect.fail(new Error(`Unknown template: ${req.templateId}`));
    ...
    const pageId = yield* buildPage(template, req.parentId, sortOrder, now, ctx);

    const rows = yield* sql.unsafe(`SELECT ${PAGE_COLS} FROM pages WHERE id = ?`, [pageId]);
