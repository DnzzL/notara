...
describe("openRecordAsPage", () => {
  ...
  it("creates a page and returns its id", async () => {
    ...
    const page = await testRun(Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      const rows = yield* sql`SELECT id, title, parent_id FROM pages WHERE id = ${result.pageId}`;
...
const pageDeletedFlag = (pageId: string) =>
  testRun(Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    const rows = yield* sql`SELECT is_deleted as "isDeleted" FROM pages WHERE id = ${pageId}`;
...
const rowCount = (table: string, where: string) =>
  testRun(Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    const rows = yield* sql.unsafe(`SELECT COUNT(*) as cnt FROM ${table} WHERE ${where}`);
...
describe("trash listing, restore, and purge sweep", () => {
  ...
  it("restoreDatabase untrashes a soft-deleted database", async () => {
    ...
    expect(await rowCount("databases", "id = 'db1' AND is_deleted = 0")).toBe(1);
  });
