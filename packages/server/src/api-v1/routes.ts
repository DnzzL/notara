		...
		handle(
			Effect.gen(function* () {
				...
				const page = yield* withWorkspace(
					workspaceId,
					Pages.createPage({ title, parentId }),
				);
				return created(page);
			}),
		),
			...
			Effect.gen(function* () {
				...
				const page = yield* withWorkspace(
					...
					Pages.updatePage({
						...
						coverUrl:
							"coverUrl" in b ? (b.coverUrl as string | null) : undefined,
	...
	yield* router.add(
		...
		handle(
			Effect.gen(function* () {
				...
				if (permanent)
					yield* withWorkspace(workspaceId, Databases.purgePage(pageId));
