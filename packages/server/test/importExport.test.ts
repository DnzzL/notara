...
import { exportPageAsMarkdown, exportDatabaseAsCsv } from "../src/export/page.js";
...
const allMigrationsPath = path.join(import.meta.dirname || __dirname, "../migrations");

function runAllMigrations(filename: string) {
  const migrationFiles = fs.readdirSync(allMigrationsPath)
    .filter((f) => f.endsWith(".sql"))
    .sort();
  const db = new Database(filename);
  try {
    for (const file of migrationFiles) {
      const sqlContent = fs.readFileSync(path.join(allMigrationsPath, file), "utf-8");
      db.exec(sqlContent);
    }
  } finally {
    db.close();
