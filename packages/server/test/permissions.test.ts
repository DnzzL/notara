...
function seedWorkspace(platformDb: Database, userId: string, userName: string, role: "owner" | "member") {
  ...
  ).run(wsId, `${userName}'s workspace`, `${userId}-ws`, userId, inviteToken, now);
  platformDb.prepare(
...
describe("Permissions", () => {
  ...
  test("workspace owner resolves to owner relation with no ACL", async () => {
    const page = await Pages.createPage({ title: "Owner Page", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );

    const rel = await Permissions.resolveEffectiveRelation(
      owner.userId, owner.workspaceId, page.id,
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);

    expect(rel).toBe("owner");
  });

  test("workspace owner can access page with viewer permission requirement", async () => {
    const page = await Pages.createPage({ title: "Owner Access", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );

    const result = await Permissions.checkPagePermission(
      owner.userId, owner.workspaceId, page.id, "viewer",
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("workspace member resolves to editor relation with no ACL", async () => {
    const page = await Pages.createPage({ title: "Member Page", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );

    const rel = await Permissions.resolveEffectiveRelation(
      member.userId, member.workspaceId, page.id,
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("workspace member can access page with editor permission requirement", async () => {
    const page = await Pages.createPage({ title: "Member Editor", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );

    const result = await Permissions.checkPagePermission(
      member.userId, member.workspaceId, page.id, "editor",
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("non-member user resolves to null", async () => {
    const page = await Pages.createPage({ title: "Stranger Page", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );

    const rel = await Permissions.resolveEffectiveRelation(
      "non-member", owner.workspaceId, page.id,
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("explicit viewer grant overrides default editor", async () => {
    const page = await Pages.createPage({ title: "Viewer Page", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("explicit editor grant on member", async () => {
    const page = await Pages.createPage({ title: "Editor Grant", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("explicit owner grant on member", async () => {
    const page = await Pages.createPage({ title: "Owner Grant", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    ...
    await Permissions.writePagePermissions({
      pageId: page.id,
      set: [{ subject: { type: "user", id: member.userId }, relation: "owner" }],
      remove: [],
    }).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("higher-rank relation overrides lower-rank when both set", async () => {
    const page = await Pages.createPage({ title: "Rank Override", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("workspace:member subject grants all workspace members", async () => {
    const page = await Pages.createPage({ title: "All Members", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    ...
    await Permissions.writePagePermissions({
      pageId: page.id,
      set: [
        { subject: { type: "workspace", id: member.workspaceId, relation: "member" }, relation: "viewer" },
        { subject: { type: "user", id: owner.userId }, relation: "owner" },
      ],
      remove: [],
    }).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("public subject grants access to any workspace member", async () => {
    const page = await Pages.createPage({ title: "Public Page", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("child page inherits ACL from parent (locked ancestor)", async () => {
    ...
    const rel = await Permissions.resolveEffectiveRelation(
      member.userId, member.workspaceId, child.id,
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("child inherits highest relation from ancestor lock", async () => {
    const parent = await Pages.createPage({ title: "Ancestor", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    const child = await Pages.createPage({ title: "Descendant", parentId: parent.id }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    const grandchild = await Pages.createPage({ title: "Grandchild", parentId: child.id }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    ...
    const rel = await Permissions.resolveEffectiveRelation(
      member.userId, member.workspaceId, grandchild.id,
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
  ...
  test("child blocked when ancestor locked and user has no grant", async () => {
    const parent = await Pages.createPage({ title: "Locked Parent", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    const child = await Pages.createPage({ title: "Blocked Child", parentId: parent.id }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("write then read ACL roundtrip", async () => {
    const page = await Pages.createPage({ title: "Roundtrip", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("revision bumps on each write", async () => {
    const page = await Pages.createPage({ title: "Revisions", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
  ...
  test("write with stale ifRevision returns 409", async () => {
    const page = await Pages.createPage({ title: "Conflict", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    ...
    const result = await Permissions.writePagePermissions({
      ...
      remove: [],
    }).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromiseExit);
  ...
  test("member sees pages with inherited or direct ACL access", async () => {
    const ownerPage = await Pages.createPage({ title: "Open", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    const restricted = await Pages.createPage({ title: "Restricted", parentId: null }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    const childOfRestricted = await Pages.createPage({ title: "Child", parentId: restricted.id }).pipe(
      Effect.provide(SqliteClient.layer({ filename: sqliteFilename })),
      Effect.runPromise,
    );
    ...
    const visible = await Permissions.filterPagesByPermission(
      member.userId, member.workspaceId, "member", allPages,
    ).pipe(Effect.provide(makeLayers(platformDb, sqliteFilename)), Effect.runPromise);
