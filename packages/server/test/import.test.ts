...
import { markdownToBlocks, parsePageTitle, determineParent } from "../src/import/notion.js";
...
describe("parsePageTitle", () => {
  ...
  test("strips GUID from filename fallback", () => {
    const md = "Content";
    expect(parsePageTitle(md, "My Page (a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4).md")).toBe("My Page");
  });
});

describe("determineParent", () => {
  test("extracts parent GUID from Notion folder structure", () => {
    const filePath = "Private & Shared/Parent Page (a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4)/Child.md";
    const guidMap = new Map([["a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4", "page-uuid-1"]]);
    expect(determineParent(filePath, guidMap)).toBe("page-uuid-1");
  });

  test("returns null for root-level files", () => {
    const filePath = "Private & Shared/Page (a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4).md";
    const guidMap = new Map();
    expect(determineParent(filePath, guidMap)).toBe(null);
  });

  test("returns null when parent GUID not in map", () => {
    const filePath = "Private & Shared/Parent (a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4)/Child.md";
    const guidMap = new Map();
    expect(determineParent(filePath, guidMap)).toBe(null);
  });
});
