...
describe("Workspaces", () => {
  ...
  test("getMyWorkspaces returns all workspaces for a user", async () => {
    await Workspaces.createWorkspace({ userId: "user-1", name: "WS A", slug: "ws-a" }).pipe(
      Effect.provide(makeLayer(db)),
      Effect.runPromise,
    );
    await Workspaces.createWorkspace({ userId: "user-1", name: "WS B", slug: "ws-b" }).pipe(
      Effect.provide(makeLayer(db)),
      Effect.runPromise,
    );
  ...
  test("getMyWorkspaces only shows owner invite token", async () => {
    ...
    // Join as member
    await Workspaces.joinWorkspaceByToken({ userId: "member-1", inviteToken: ws.inviteToken! }).pipe(
      Effect.provide(makeLayer(db)),
      Effect.runPromise,
    );
  ...
  test("removeMember removes a member (owner cannot be removed)", async () => {
    ...
    await Workspaces.removeMember({ workspaceId: ws.id, userId: "member-1" }).pipe(
      Effect.provide(makeLayer(db)),
      Effect.runPromise,
    );
  ...
  test("regenerateInviteLink returns new token and old token is invalidated", async () => {
    ...
    const { inviteToken: newToken } = await Workspaces.regenerateInviteLink(ws.id).pipe(
      Effect.provide(makeLayer(db)),
      Effect.runPromise,
    );
