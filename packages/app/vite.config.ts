  ...
  return {
    plugins: [
      ...
      manifest: {
        name: "Notara",
        short_name: "Notara",
        description: "Your second brain — pages, databases, and rich documents.",
        theme_color: "#ffffff",
      ...
      workbox: {
        ...
        runtimeCaching: [
          {
            urlPattern: /^\/api\/(?!auth\/(sign-in|sign-up|request-password-reset|reset-password)).*/,
        ...
        navigateFallbackDenylist: [/^\/api/, /^\/attachments/, /^\/import-notion/],
      },
    }),
  ],
  server: {
    ...
    proxy: {
      ...
      "/import-notion": { target: "http://127.0.0.1:3000", changeOrigin: true },
