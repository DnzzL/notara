import type { Backlink, Page, SearchResult } from "@notara/shared";
...
export interface PageState {
	...
	importNotion: (
		directory: string,
	) => Promise<{ pagesImported: number; databasesImported: number }>;
}
...
export const usePageStore = create<PageState>((set, get) => ({
	...
	selectPage: (page) => {
		...
		const filtered = [
			page.id,
			...recent.filter((x: string) => x !== page.id),
		].slice(0, 5);
	...
	loadRecentPages: async () => {
		const ids: string[] = (() => {
			try {
				return JSON.parse(localStorage.getItem("notara:recentPages") || "[]");
			} catch {
				return [];
			}
		})();
