...
function WorkspaceSettingsPage() {
	...
	useEffect(() => {
		api.getMyWorkspaces().then((ws) => {
			...
			api
				.getWorkspaceMembers({ workspaceId: found.id })
				.then(setMembers)
	...
	const handleRegenerate = async () => {
		if (!workspace) return;
		const { inviteToken: newToken } = await api.regenerateInviteLink({
	...
	if (notFound) {
		return (
			<div className="admin-forbidden">
				...
				<button onClick={() => navigate({ to: "/" })} className="auth-toggle">
					Go home
				</button>
			</div>
		);
								...
								{isOwner && (
									<section className="mb-5">
										...
										<div className="mt-3.5">
											<h4 className="text-[12px] font-semibold text-text-3 uppercase tracking-[0.05em] mb-2">
