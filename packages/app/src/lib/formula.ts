...
function tokenize(src: string): Tok[] {
	...
	while (i < src.length) {
		...
		if (/[0-9.]/.test(c)) {
			...
			if (Number.isNaN(n))
				throw new FormulaError(`Bad number: ${src.slice(i, j)}`);
			out.push({ t: "num", v: n });
			i = j;
			continue;
		}
...
class Parser {
	...
	parseFactor(): Node {
		...
		if (t.t === "lp") {
			...
			if (r?.t !== "rp") throw new FormulaError("Missing closing paren");
			return e;
		}
		if (t.t === "ident") {
			this.consume();
			const lp = this.peek();
			if (lp?.t !== "lp")
				throw new FormulaError(`Identifier '${t.v}' must be followed by '('`);
			this.consume();
			const args: Node[] = [];
			if (this.peek()?.t !== "rp") {
			...
			if (rp?.t !== "rp") throw new FormulaError("Missing closing paren");
		...
		case "bin": {
			...
			switch (n.op) {
				...
				case "<=":
					return toNum(av) <= toNum(bv);
				case "==":
					return av === bv || toNum(av) === toNum(bv);
				case "!=":
					return !(av === bv || toNum(av) === toNum(bv));
			}
			...
			switch (n.name) {
				case "prop": {
					...
					if (typeof key !== "string")
						throw new FormulaError("prop() requires a string argument");
				...
				case "if": {
					if (args.length !== 3)
						throw new FormulaError("if() requires 3 arguments");
					return truthy(args[0]) ? args[1] : args[2];
				}
				case "sum":
					return args.reduce<number>((acc, v) => acc + toNum(v), 0);
				case "round": {
