...
export function BackupsPanel() {
	...
	}, [settings.s3Enabled, loadBackups]);
	...
	const handleBackup = async () => {
		...
		try {
			...
			setBackupMessage(
				`Backed up to ${data.key} (${(data.size / 1024).toFixed(0)} KB)`,
			);
			toaster.create({
				title: "Backup complete",
				description: `Saved to ${data.key}`,
				type: "success",
			});
		} catch (err) {
	...
	const handleRestore = async () => {
		...
		if (
			...
			return;
		if (prompt("Type RESTORE to confirm overwriting all data:") !== "RESTORE")
			return;
				...
				<>
					<div className="settings-fields">
						...
						<label className="settings-field">
							...
							<select
								...
								onChange={(e) =>
									set("s3Schedule", e.target.value as BackupSchedule)
								}
								className="settings-select"
							>
								{(
									Object.entries(SCHEDULE_LABELS) as [BackupSchedule, string][]
								).map(([v, label]) => (
					...
					<div className="settings-backup-row">
						{backupStatus === "success" && (
							<span className="settings-backup-msg success">
								{backupMessage}
							</span>
						)}
						...
						{backupStatus === "idle" && lastBackup && (
							<span className="settings-last-backup">
					...
					<p className="settings-field-hint">
						Replaces <strong>all</strong> workspaces and attachments with the
						contents of the selected backup. A snapshot of the current state is
						saved to S3 first, then the server restarts.
					</p>
					...
					<div className="settings-backup-row">
						{restoreStatus !== "idle" && restoreMessage && (
							<span
								className={`settings-backup-msg${restoreStatus === "done" ? " success" : ""}`}
							>
								{restoreMessage}
							</span>
						)}
