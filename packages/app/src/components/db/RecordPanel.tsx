...
export function RecordPanel({
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(15,18,30,0.3)] backdrop-blur-[4px] z-[9000] flex justify-end [animation:fade-in_0.14s_var(--ease)]" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <aside className="w-[min(520px,90vw)] h-full bg-surface border-l border-border-mid shadow-[var(--shadow-xl)] flex flex-col px-7 pb-7 pt-[18px] overflow-y-auto [animation:slide-from-right_0.18s_var(--ease-spring)] [&::-webkit-scrollbar]:w-[5px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-surface-4 [&::-webkit-scrollbar-thumb]:rounded-[3px]" role="dialog" aria-label="Record details">
        ...
        <section className="grid grid-cols-[130px_1fr] gap-y-1.5 gap-x-3 items-start mb-6">
              ...
              <div key={field.id} className="contents">
                ...
                <div className="record-panel-prop-value relative min-h-[28px]">
                  {isEditing ? (
                    ...
                    <div className="px-[7px] py-1 rounded-[5px] cursor-pointer min-h-[24px] text-text-2 transition-[background,color] duration-[var(--t)] ease-[var(--ease)] hover:bg-surface-3 hover:text-text" onClick={() => setEditingFieldId(field.id)}>
          ...
          {fields.length === 0 && (
            <div className="col-span-2 text-text-3 text-[13px] pt-2">No properties yet. Add one from the table header.</div>
          )}
        </section>
      </aside>
    </div>
