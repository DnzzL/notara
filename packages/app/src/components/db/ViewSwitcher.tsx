...
import { useDatabaseStore, selectDbViews, selectActiveViewId, parseViewConfig, serializeViewConfig } from "../../stores/databaseStore.js";
import { selectFilters, selectSorts, selectBoardHidden, selectBoardGroupBy } from "../../stores/databaseStore.js";
...
export function ViewSwitcher({
  ...
  const activeViewId = useDatabaseStore((s) => selectActiveViewId(s, databaseId));
  ...
  const boardGroupBy = useDatabaseStore((s) => selectBoardGroupBy(s, databaseId));
  ...
  useEffect(() => {
    if (!open) { setRenamingId(null); setSavingAs(false); return; }
  ...
  const handleSelect = useCallback((view: DatabaseView | null) => {
  ...
  const handleRename = useCallback(async (viewId: string) => {
  ...
  const handleDelete = useCallback(async (view: DatabaseView) => {
        ...
        >
          ...
          <button
            ...
            className={cn(
              ...
              !activeView ? "text-text font-medium" : "text-text-2 hover:bg-surface-3",
            )}
          >
            ...
            <span>All</span>
            {!activeView && <span style={{ marginLeft: "auto", color: "var(--accent, #2eaadc)" }}>✓</span>}
          </button>

          {dbViews.length > 0 && <div style={{ height: 1, background: "var(--border, #e9e9e7)", margin: "3px 8px" }} />}

          {/* Saved views */}
          {dbViews.map((view) => (
            <div key={view.id} className="flex items-center group" style={{ position: "relative" }}>
              {renamingId === view.id ? (
                <input
                  ref={renameInputRef}
                  type="text"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") { e.preventDefault(); handleRename(view.id); }
                    if (e.key === "Escape") { setRenamingId(null); setRenameValue(""); }
                  }}
                  style={{
                    flex: 1, margin: "2px 3px", padding: "2px 6px", fontSize: 13,
                    border: "1px solid #2eaadc", borderRadius: 4, outline: "none",
                  }}
                ...
                <button
                  ...
                  className={cn(
                    "w-full bg-transparent border-none text-left px-3 py-1.5 text-[13px] cursor-pointer flex items-center gap-2 transition-[background] duration-[var(--t)] ease-[var(--ease)]",
                    activeViewId === view.id ? "text-text font-medium" : "text-text-2 hover:bg-surface-3",
                  )}
                >
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" style={{ opacity: 0.6 }}>
                    {view.type === "board" ? (
          ...
          <div style={{ height: 1, background: "var(--border, #e9e9e7)", margin: "3px 8px" }} />
            ...
            <div style={{ padding: "4px 8px" }}>
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape") { setSavingAs(false); setSaveName(""); }
                }}
