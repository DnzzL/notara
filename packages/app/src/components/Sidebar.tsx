...
export function Sidebar({
	...
	}, [filtered.length, filtered.filter, filtered.map]);
	...
	}, [pageMap.get, currentPage?.parentId]);
	...
	const collection = useMemo(
		() =>
			createTreeCollection<TreeNodeData>({
				...
				nodeToString: (n: TreeNodeData) =>
					pageMap.get(n.id)?.title ?? "Untitled",
	...
	const zoneFor = (over: NonNullable<DragOverEvent["over"]>): DropZone => {
		...
		const ratio =
			overRect.height > 0
				? (pointerYRef.current - overRect.top) / overRect.height
				: 0.5;
	...
	const handleDragEnd = useCallback(
		async ({ active, over }: DragEndEvent) => {
			...
			if (zone === "nest") {
				...
				setExpandedValue((prev) =>
					prev.includes(overId) ? prev : [...prev, overId],
				);
				return;
			}
		...
		[filtered, pages, movePage, reorderPages, zoneFor],
	);
	...
	return (
		...
		>
			<SortableContext items={treeOrder} strategy={verticalListSortingStrategy}>
				...
				>
					...
					<nav className="flex-1 overflow-y-auto overflow-x-hidden px-1.5 pt-0.5 pb-3.5 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-sb-3 [&::-webkit-scrollbar-thumb]:rounded-sm [&::-webkit-scrollbar-thumb:hover]:bg-sb-4">
							...
							<>
								...
								<div className="flex flex-col">
									{favorites.map((page) => (
										...
										>
											<span className="w-2" />
											<span className="text-[13px] leading-none shrink-0 cursor-pointer w-[18px] text-center">
												{page.icon || "📄"}
											</span>
											<span className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap tracking-[-0.005em]">
												{page.title || "Untitled"}
											</span>
										</div>
									))}
								</div>
								<div className="text-[11px] font-medium text-text-sb-3 px-2 pt-2.5 pb-[3px] flex items-center gap-1 mt-1.5">
									Pages
								</div>
							</>
						)}
						...
						{loading ? (
							<div className="px-2 py-3 text-text-sb-3 text-[12px]">
								Loading…
							</div>
							...
							<div className="px-2.5 py-3.5 text-text-sb-3 text-[12.5px] flex flex-col gap-2 items-start leading-relaxed">
								...
								<button
									className="px-3 py-[5px] bg-accent text-white border-none rounded-lg cursor-pointer text-[12px] font-medium transition-opacity duration-[var(--t)] ease-[var(--ease)] hover:opacity-[0.88]"
									...
									{collection.rootNode.children?.map(
										(node: TreeNodeData, index: number) => (
											<PageTreeNode
												...
												onIconClick={(pageId, coords) =>
													setIconPickerFor({ pageId, ...coords })
												}
												depth={0}
			...
			<EmojiPicker
				...
				onSelect={(icon) =>
					iconPickerFor && setPageIcon(iconPickerFor.pageId, icon)
				}
			/>
			...
			{showTrash && (
				<TrashModal onClose={() => setShowTrash(false)} onChanged={loadPages} />
			)}
			{showTemplatePicker && (
				<TemplatePicker
...
function PageTreeNode({
	...
	const _isActive = activePageId === node.id;
	...
	return (
		<TreeView.NodeProvider node={node} indexPath={indexPath}>
			<div
				...
				className={
					depth > 0
						? "relative before:content-[''] before:absolute before:left-[7px] before:top-0 before:bottom-0 before:w-px before:bg-border-sb"
						: undefined
				}
			>
				<TreeView.Branch>
