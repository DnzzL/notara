  ...
  return (
    <Modal title="API keys" onClose={onClose} className="max-w-[560px]">
      <p className="text-[13.5px] text-text-2 leading-relaxed mb-4">
        ...
        <a href="/docs" target="_blank" rel="noopener noreferrer" className="text-accent no-underline font-medium hover:opacity-75">
          API docs ↗
        </a>
      </p>
      ...
      {created && (
        <div className="bg-[rgba(22,163,74,0.06)] border border-[rgba(22,163,74,0.25)] rounded p-3.5 mb-4">
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-[13px] font-semibold text-success">✓ Key created — copy it now, it won't be shown again</span>
            <button className="bg-transparent border-none cursor-pointer text-text-3 text-sm px-0.5" onClick={() => setCreated(null)}>✕</button>
          </div>
          <div className="flex gap-2 items-center">
            ...
            <Button variant="primary" size="sm" onClick={handleCopy}>{copied ? "Copied!" : "Copy"}</Button>
          </div>
        </div>
      )}

      <section className="mb-5">
        <h3 className="text-[11.5px] font-semibold mb-2.5 text-text-3 uppercase tracking-[0.06em]">New key</h3>
      ...
      <section className="mb-5">
        <h3 className="text-[11.5px] font-semibold mb-2.5 text-text-3 uppercase tracking-[0.06em]">Active keys</h3>
        {apiKeysLoading ? (
          ...
          <div className="flex flex-col items-center text-center px-4 pt-7 pb-3 [animation:empty-state-in_0.4s_var(--ease-spring)]">
            <div className="mb-4 [filter:drop-shadow(0_4px_12px_rgba(43,77,255,0.12))]" aria-hidden="true">
            ...
            <p className="[font-family:var(--font-title)] text-[17px] font-bold text-text tracking-[-0.02em] mb-1.5">No keys yet</p>
            <p className="text-[13.5px] text-text-3 leading-relaxed max-w-[280px]">
              Create a key above to start automating Notara from scripts or CI pipelines.
            </p>
          </div>
        ) : (
          <ul className="list-none m-0 p-0 flex flex-col gap-0.5">
            {apiKeys.map((k) => (
              <li key={k.id} className="flex flex-col gap-1 px-3.5 py-3 border border-border rounded bg-surface">
                <div className="flex items-center gap-2.5">
                  <span className="font-semibold text-sm text-text">{k.name}</span>
                  <code className="[font-family:var(--font-mono)] text-[12px] bg-surface-3 rounded px-1.5 py-0.5 text-text-3">{k.keyPrefix}…</code>
                </div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Button variant="danger" size="sm" onClick={() => handleRevoke(k.id, k.name)}>Revoke</Button>
                </div>
              </li>
            ))}
          </ul>
