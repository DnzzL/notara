...
export function SlashMenu({
	...
	return (
		...
		>
			<div className="px-2.5 pt-2 pb-1 text-[10.5px] font-semibold text-text-3 uppercase tracking-[0.07em]">
				Blocks
			</div>
			{filteredCommands.map((cmd, index) => (
				...
				>
					<span className="flex items-center justify-center w-7 h-7 bg-surface-3 border border-border rounded-lg text-[13px] shrink-0">
						{cmd.icon}
					</span>
					<div className="flex items-center flex-1 justify-between gap-2">
						<span className="font-medium">{cmd.name}</span>
						{cmd.shortcut && (
							<span className="text-[11px] text-text-3 [font-family:var(--font-mono)]">
								{cmd.shortcut}
							</span>
						)}
					</div>
