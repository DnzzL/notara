    ...
    <div className="py-1" data-block-id={block.id}>
      {userIds.length > 0 && !pickerOpen && (
        <div className="flex flex-wrap gap-1.5 items-center">
          {userIds.map((uid) => {
            ...
            return (
              <span
                ...
                title="Click to edit"
              >
                <span className="inline-flex items-center justify-center w-[22px] h-[22px] rounded-full bg-accent-dim text-accent-2 text-[11px] font-semibold shrink-0">{initial}</span>
          ...
          <button className="bg-transparent border-[1.5px] border-dashed border-border-mid rounded-[20px] w-7 h-7 inline-flex items-center justify-center cursor-pointer text-text-3 text-[16px] transition-[border-color,color] duration-[var(--t)] ease-[var(--ease)] hover:border-accent hover:text-accent" onClick={() => setPickerOpen(true)}>+</button>
        </div>
      )}
      {pickerOpen && (
        <div className="bg-surface border border-border-mid rounded-lg p-1.5 shadow-[var(--shadow-lg)] max-h-[280px] flex flex-col" onMouseDown={(e) => e.stopPropagation()}>
          ...
          <div className="flex-1 overflow-y-auto mt-1">
            {visible.length === 0 ? (
              <div className="px-2 py-2.5 text-text-3 text-[13px]">No people found</div>
            ) : visible.map((m) => {
