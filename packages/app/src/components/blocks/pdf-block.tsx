...
export function PdfBlock({ block }: BlockRendererProps) {
  ...
  const data = tryParseBlockContent<{ src: string; fileName?: string; size?: number }>(block.content ?? "");
  if (data) {
    src = data.src;
    ...
    <div className="block-pdf">
      <div className="block-pdf-card">
        ...
        <div className="block-pdf-card-actions">
          ...
          <a href={src} target="_blank" rel="noopener noreferrer" className="block-pdf-card-btn">Open</a>
        </div>
      </div>
      {previewOpen && (
