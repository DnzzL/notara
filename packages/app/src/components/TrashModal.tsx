		...
		try {
			if (op === "restore") {
				await (kind === "page"
					? api.restorePage({ id })
					: kind === "database"
						? api.restoreDatabase({ id })
						: api.restoreRecord({ id }));
			} else {
				await (kind === "page"
					? api.purgePage({ id })
					: kind === "database"
						? api.purgeDatabase({ id })
						: api.purgeRecord({ id }));
			}
			await load();
			onChanged?.();
			...
			<div className="flex items-center justify-between gap-2">
				...
				<div className="flex gap-1.5">
					<Button
						...
						size="sm"
						disabled={busy === id}
						onClick={() => act("restore", kind, id, label)}
					>
						Restore
					</Button>
					<Button
						...
						size="sm"
						disabled={busy === id}
						onClick={() => act("purge", kind, id, label)}
					>
						Delete forever
					</Button>
				</div>
				...
				<>
					{trash.pages.length > 0 && (
						<section className="mb-5">
							...
							<ul className="list-none m-0 p-0 flex flex-col gap-0.5">
								{trash.pages.map((p) =>
									row("page", p.id, p.title, p.deletedAt),
								)}
							</ul>
						</section>
					)}
						...
						<section className="mb-5">
							...
							<ul className="list-none m-0 p-0 flex flex-col gap-0.5">
								{trash.databases.map((d) =>
									row("database", d.id, d.name, d.deletedAt),
								)}
							</ul>
						</section>
					)}
					{trash.records.length > 0 && (
						<section className="mb-5">
							...
							<ul className="list-none m-0 p-0 flex flex-col gap-0.5">
								{trash.records.map((r) =>
									row("record", r.id, r.title, r.deletedAt),
								)}
							</ul>
						</section>
					)}
