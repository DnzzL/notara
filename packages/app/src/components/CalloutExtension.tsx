      ...
      {anchor !== null &&
        createPortal(
          <EmojiPicker
            ...
            onSelect={(emoji) => updateAttributes({ icon: emoji || DEFAULT_ICON })}
          />,
          document.body,
        )}
    </NodeViewWrapper>
  ...
  addAttributes() {
    return {
      icon: {
        ...
        parseHTML: (el: HTMLElement) => el.getAttribute("data-icon") || DEFAULT_ICON,
        renderHTML: (attrs: { icon?: string }) => ({ "data-icon": attrs.icon || DEFAULT_ICON }),
      },
    };
  },
