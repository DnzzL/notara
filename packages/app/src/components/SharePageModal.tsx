...
function isUserEntry(
	e: AclEntry,
): e is AclEntry & { subject: { type: "user"; id: string } } {
...
export function SharePageModal({ pageId, workspaceId, onClose }: Props) {
	...
	const handleShareToggle = async () => {
		...
		try {
			if (shareToken) {
				await api.setPageSharing({ pageId, enabled: false });
				setShareToken(null);
				toaster.create({ title: "Public link removed", type: "success" });
			} else {
				const result = await api.setPageSharing({ pageId, enabled: true });
				setShareToken(result.token);
				toaster.create({ title: "Public link created", type: "success" });
			}
		} catch (err: any) {
			toaster.create({
	...
	const handleCopyUrl = async () => {
		...
		try {
			...
			toaster.create({ title: "Copy the URL manually", type: "info" });
		}
	};
				...
				<>
					...
					<section className="mb-5">
						<div className="flex justify-between items-center mb-2.5">
							<h3 className="text-[11.5px] font-semibold text-text-3 uppercase tracking-[0.06em]">
								People with access
							</h3>
								...
								{availableMembers.map((m) => (
									...
									>
										...
										<div className="flex-1">
											<span className="text-[13.5px] font-medium block text-text">
												{m.name || m.email}
											</span>
						...
						<ul className="list-none p-0 m-0 flex flex-col gap-1">
										...
										<select
											...
											onChange={(ev) =>
												handleChangeRelation(
													e.subject,
													ev.target.value as AclRelation,
												)
											}
										>
											{RELATIONS.map((r) => (
							...
							{directUserEntries.length === 0 && (
								<li className="px-1 py-2 text-[12px] text-text-3 list-none">
									...
									return (
										...
										>
											<div className="flex-1">
												...
												<span className="text-[11.5px] text-text-3 capitalize">
													{e.relation} (inherited)
												</span>
											</div>
										</li>
									);
					...
					<section className="mb-5">
						...
						<div className="flex items-center justify-between gap-3 px-3 py-2.5 bg-surface-2 border border-border rounded-lg">
							<div className="flex-1">
								<p className="m-0 text-[13px] text-text">Public access</p>
								<p className="m-0 text-[11.5px] text-text-3 mt-0.5">
									{shareToken
						...
						{shareToken && (
							<div className="mt-2 flex items-center gap-2 px-3 py-2 bg-surface-2 border border-border rounded-lg">
								<code className="flex-1 text-[12px] text-text-2 truncate select-all">
									{window.location.origin}/p/{shareToken}
								</code>
								<button
									className="shrink-0 px-2 py-1 text-[11px] font-medium border border-border rounded bg-surface text-text-2 cursor-pointer hover:bg-surface-3 transition-colors"
					...
					<section className="mb-5">
						...
						<label className="flex items-center gap-2 text-[13px] text-text-2 cursor-pointer">
							...
							<select
								...
								onChange={(e) =>
									handleSetWorkspaceAccess(
										e.target.value as AclRelation | "off",
									)
								}
							>
