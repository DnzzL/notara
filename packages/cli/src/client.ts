...
const ctor: Record<Method, (url: string) => HttpClientRequest.HttpClientRequest> = {
...
export const requireWorkspace = (cfg: Cfg): Effect.Effect<string, NotaraError> =>
  cfg.workspace
...
export const request = (
  ...
  Effect.gen(function* () {
    ...
    if (options.query) req = req.pipe(HttpClientRequest.setUrlParams(options.query));
    if (options.body !== undefined) req = yield* HttpClientRequest.bodyJson(req, options.body);
    ...
    if (res.status >= 400) {
      ...
      return yield* new NotaraError({ message: `HTTP ${res.status}: ${message}` });
    }
