...
const pagesUpdate = Command.make(
	"update",
	{
		...
		icon: Options.text("icon").pipe(
			Options.optional,
			Options.withDescription("New icon (emoji)"),
		),
...
const blockId = Args.text({ name: "blockId" }).pipe(
	Args.withDescription("Block id (ULID)"),
);
...
const blocksUpdate = Command.make(
	"update",
	{
		...
		blockId,
		content: Options.text("content").pipe(
			Options.withDescription("New block content"),
		),
	},
	(o) =>
		Effect.gen(function* () {
...
const dbId = Args.text({ name: "dbId" }).pipe(
	Args.withDescription("Database id"),
);
const fieldId = Args.text({ name: "fieldId" }).pipe(
	Args.withDescription("Field id"),
);
const recordId = Args.text({ name: "recordId" }).pipe(
	Args.withDescription("Record id"),
);
...
const databasesCreate = Command.make(
	...
	{
		...globals,
		page: Options.text("page").pipe(
			Options.withDescription("Page id to create the database on"),
		),
...
const databasesUpdate = Command.make(
	"update",
	{
		...
		name: Options.text("name").pipe(
			Options.optional,
			Options.withDescription("New database name"),
		),
...
const databasesAddField = Command.make(
	"add-field",
	{
		...
		type: Options.text("type").pipe(
			...
			Options.withDescription(fieldTypeDesc),
		),
...
const databasesUpdateField = Command.make(
	"update-field",
	{
		...
		name: Options.text("name").pipe(
			Options.optional,
			Options.withDescription("New field name"),
		),
		type: Options.text("type").pipe(
			Options.optional,
			Options.withDescription(fieldTypeDesc),
...
const databasesAddRecord = Command.make(
	"add-record",
	{
		...globals,
		dbId,
		title: Options.text("title").pipe(Options.withDescription("Record title")),
	},
	(o) =>
		Effect.gen(function* () {
			const cfg = toCfg(o);
...
const searchCmd = Command.make(
	"search",
	{
		...
		query: Args.text({ name: "query" }).pipe(
			Args.withDescription("Search text"),
		),
	},
	(o) =>
		Effect.gen(function* () {
